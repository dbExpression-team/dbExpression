    ...
    public class NullableRoundFunctionExpressionCodeGenerator : CodeGenerator<FunctionTemplateModel>
    {
        ...
        public override void Generate(string templatePath, string outputSubdirectory)
        {
            foreach (var @type in TypeBuilder.CreateBuilder().AddNumericTypes().Add<object?>().Except(typeof(byte), typeof(short)).ToList())
                Generate(templatePath, outputSubdirectory, $"Nullable{@type.Name}{functionName}FunctionExpression.generated.cs", CreateModel("DbExpression.MsSql.Expression", @type));
        }
    }
}

...
{
    public class NullableAverageFunctionExpressionCodeGenerator : CodeGenerator<FunctionTemplateModel>
    {
        private const string functionName = "Average";
        ...
        {
            foreach (var @type in TypeBuilder.CreateBuilder().AddNumericTypes().Except(typeof(byte), typeof(short)).Add<object?>().ToList())

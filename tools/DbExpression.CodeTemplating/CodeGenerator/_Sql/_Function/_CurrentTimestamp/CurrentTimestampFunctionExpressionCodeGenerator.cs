...
namespace DbExpression.CodeTemplating.CodeGenerator
{
    public class CurrentTimestampFunctionExpressionCodeGenerator : CodeGenerator<FunctionTemplateModel>
    {
        ...
        {
            Generate(templatePath, outputSubdirectory, $"{functionName}FunctionExpression.generated.cs", CreateModel("DbExpression.Sql.Expression", TypeBuilder.CreateBuilder().Add<DateTime>().ToList().Single()));
        }
    }
}

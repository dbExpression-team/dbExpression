...
{
    public class StubTypedFieldExpressionCodeGenerator : CodeGenerator<TemplateModel>
    {
        public override void Generate(string templatePath, string outputSubdirectory)
        {
            foreach (var @type in TypeBuilder.CreateBuilder().AddAllTypes().ToList())
                Generate(templatePath, outputSubdirectory, $"{@type.Name}FieldExpression.cs", CreateModel("HatTrick.DbEx.Sql.Expression", @type));
        }
    }
}

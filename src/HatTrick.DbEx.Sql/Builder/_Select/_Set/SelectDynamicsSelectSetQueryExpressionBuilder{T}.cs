...
{
    public class SelectDynamicsSelectSetQueryExpressionBuilder<TDatabase> : SelectSetQueryExpressionBuilder<TDatabase>,
        SelectDynamicsTermination<TDatabase>
        where TDatabase : class, ISqlDatabaseRuntime
    {
        #region constructors
        public SelectDynamicsSelectSetQueryExpressionBuilder(
        ...
        protected virtual IList<dynamic> ExecutePipeline(ISqlConnection connection, Action<IDbCommand>? configureCommand)
        {
            if (SelectSetQueryExpression.Expressions.Count > 1)
                return CreateExecuteSetPipeline().ExecuteSelectDynamicList(SelectSetQueryExpression, connection, configureCommand);
            return CreateExecuteSinglePipeline().ExecuteSelectDynamicList(SelectSetQueryExpression.Expressions.Single().SelectQueryExpression, connection, configureCommand);
        }
        ...
        {
            if (SelectSetQueryExpression.Expressions.Count > 1)
                CreateExecuteSetPipeline().ExecuteSelectDynamicList(SelectSetQueryExpression, connection, configureCommand, read);
            CreateExecuteSinglePipeline().ExecuteSelectDynamicList(SelectSetQueryExpression.Expressions.Single().SelectQueryExpression, connection, configureCommand, read);
        }
        ...
        {
            if (SelectSetQueryExpression.Expressions.Count > 1)
                return CreateExecuteSetPipeline().ExecuteSelectObjectList(SelectSetQueryExpression, connection, configureCommand, map);
            return CreateExecuteSinglePipeline().ExecuteSelectObjectList(SelectSetQueryExpression.Expressions.Single().SelectQueryExpression, connection, configureCommand, map);
        }
        ...
        {
            if (SelectSetQueryExpression.Expressions.Count > 1)
                return await CreateExecuteSetPipeline().ExecuteSelectDynamicListAsync(SelectSetQueryExpression, connection, configureCommand, cancellationToken).ConfigureAwait(false);
            return await CreateExecuteSinglePipeline().ExecuteSelectDynamicListAsync(SelectSetQueryExpression.Expressions.Single().SelectQueryExpression, connection, configureCommand, cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            if (SelectSetQueryExpression.Expressions.Count > 1)
                await CreateExecuteSetPipeline().ExecuteSelectDynamicListAsync(SelectSetQueryExpression, connection, configureCommand, read, cancellationToken).ConfigureAwait(false);
            await CreateExecuteSinglePipeline().ExecuteSelectDynamicListAsync(SelectSetQueryExpression.Expressions.Single().SelectQueryExpression, connection, configureCommand, read, cancellationToken).ConfigureAwait(false);
        }
        ...
        {
            if (SelectSetQueryExpression.Expressions.Count > 1)
                return await CreateExecuteSetPipeline().ExecuteSelectObjectListAsync(SelectSetQueryExpression, connection, configureCommand, read, cancellationToken).ConfigureAwait(false);
            return await CreateExecuteSinglePipeline().ExecuteSelectObjectListAsync(SelectSetQueryExpression.Expressions.Single().SelectQueryExpression, connection, configureCommand, read, cancellationToken).ConfigureAwait(false);
        }
        #endregion
    }
}

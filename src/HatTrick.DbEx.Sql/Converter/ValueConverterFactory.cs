...
namespace HatTrick.DbEx.Sql.Converter
{
    public class ValueConverterFactory : IValueConverterFactory
    {
        ...
        private static readonly PrimitiveValueConverter<bool?> _nullableBoolConverter = new PrimitiveValueConverter<bool?>((f) => f is null ? default(bool?) : Convert.ToBoolean(f), x => (object)x ?? DBNull.Value);
        ...
        private static readonly PrimitiveValueConverter<short?> _nullableShortConverter = new PrimitiveValueConverter<short?>((f) => f is null ? default(short?) : Convert.ToInt16(f), x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<int> _intConverter = new PrimitiveValueConverter<int>(Convert.ToInt32, x => x);
        private static readonly PrimitiveValueConverter<int?> _nullableIntConverter = new PrimitiveValueConverter<int?>((f) => f is null ? default(int?) : Convert.ToInt32(f), x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<long> _longConverter = new PrimitiveValueConverter<long>(Convert.ToInt64, x => x);
        private static readonly PrimitiveValueConverter<long?> _nullableLongConverter = new PrimitiveValueConverter<long?>((f) => f is null ? default(long?) : Convert.ToInt64(f), x => (object)x ?? DBNull.Value);
        ...
        private static readonly PrimitiveValueConverter<double?> _nullableDoubleConverter = new PrimitiveValueConverter<double?>((f) => f is null ? default(double?) : Convert.ToDouble(f), x => (object)x ?? DBNull.Value);
        ...
        private static readonly PrimitiveValueConverter<decimal?> _nullableDecimalConverter = new PrimitiveValueConverter<decimal?>((f) => f is null ? default(decimal?) : Convert.ToDecimal(f), x => (object)x ?? DBNull.Value);
        ...
        private static readonly PrimitiveValueConverter<float?> _nullableFloatConverter = new PrimitiveValueConverter<float?>((f) => f is null ? default(float?) : Convert.ToSingle(f), x => (object)x ?? DBNull.Value);
        ...
        private static readonly PrimitiveValueConverter<DateTime?> _nullableDateTimeConverter = new PrimitiveValueConverter<DateTime?>((f) => f is null ? default(DateTime?) : Convert.ToDateTime(f), x => x ?? (object)DBNull.Value);
        private static readonly PrimitiveValueConverter<DateTimeOffset> _dateTimeOffsetConverter = new PrimitiveValueConverter<DateTimeOffset>((f) => new DateTimeOffset(Convert.ToDateTime(f)), x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<DateTimeOffset?> _nullableDateTimeOffsetConverter = new PrimitiveValueConverter<DateTimeOffset?>((f) => f is null ? default(DateTimeOffset?) : new DateTimeOffset(Convert.ToDateTime(f)), x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<Guid> _guidConverter = new PrimitiveValueConverter<Guid>((f) => (Guid)f, x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<Guid?> _nullableGuidConverter = new PrimitiveValueConverter<Guid?>((f) => (Guid?)f, x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<string> _stringConverter = new PrimitiveValueConverter<string>(Convert.ToString, x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<byte[]> _byteArrayConverter = new PrimitiveValueConverter<byte[]>((f) => f is null ? default : (byte[])f, x => (object)x ?? DBNull.Value);
        private static readonly PrimitiveValueConverter<object> _objectConverter = new PrimitiveValueConverter<object>(o => o, x => (object)x ?? DBNull.Value);

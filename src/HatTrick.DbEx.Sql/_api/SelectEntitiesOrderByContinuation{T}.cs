...
#pragma warning restore IDE1006 // Naming Styles
        ...
        where TEntity : class, IDbEntity, new()
    {
        ...
        SelectEntitiesOrderByContinuation<TEntity> GroupBy(params AnyGroupByExpression[] groupBy);
        ...
        /// <param name="groupBy">A list of expressions of type <see cref="AnyGroupByExpression"/> specifying how to group the selected results.</param>
        /// <returns><see cref="SelectEntitiesOrderByContinuation{TEntity}"/>, a fluent continuation for the construction of a sql SELECT query expression for a list of <typeparamref name="TEntity"/> entities.</returns>
        SelectEntitiesOrderByContinuation<TEntity> GroupBy(IEnumerable<AnyGroupByExpression>? groupBy);
        ...
        /// <returns><see cref="SelectEntitiesOrderByContinuation{TEntity}"/>, a fluent continuation for the construction of a sql SELECT query expression for a list of <typeparamref name="TEntity"/> entities.</returns>
        SelectEntitiesOrderByContinuation<TEntity> Having(AnyHavingExpression? having);
    }
}

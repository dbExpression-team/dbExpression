namespace HatTrick.DbEx.Sql.Expression
{
    public class NullableDecimalSelectExpression : SelectExpression<decimal?>,
        NullableDecimalElement
    {
        public NullableDecimalSelectExpression(IExpressionElement<decimal,decimal?> expression)
            : base(expression)
        {

        }

        #region as
        public NullableDecimalElement As(string alias)
        {
            Alias = alias;
            return this;
        }
        #endregion

        #region order by
        OrderByExpression AnyElement.Asc => throw new DbExpressionException("Select expressions cannot be used in Order By clauses.");

        OrderByExpression AnyElement.Desc => throw new DbExpressionException("Select expressions cannot be used in Order By clauses.");
        #endregion
    }
}

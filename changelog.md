...
- Tools update is required.  Uninstall `HatTrick.DbEx.Tools` and install `DbExpression.Tools` (see [docs](https://www.dbexpression.com/docs/getting-started) for install example)
...
- If dbExpression is used statically using the db accessor, additional configuration is required ("runtime") in the config file.  (See "runtime" in [docs](https://www.dbexpression.com/docs/reference/configuration/scaffolding/general-options))

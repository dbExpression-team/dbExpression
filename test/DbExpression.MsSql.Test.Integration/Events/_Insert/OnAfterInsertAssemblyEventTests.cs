...
namespace DbExpression.MsSql.Test.Integration.Events
{
    public class OnAfterInsertAssemblyEventTests : ResetDatabaseAfterEveryTest
    {
        ...
        public Task Does_after_insert_assembly_event_fire_when_async_action_configured_with_sync_execute_while_selecting_entity()
        {
            ...
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterInsertAssembly(async _ => { actionExecuted = true; await Task.CompletedTask; }));
        ...
        public Task Does_after_insert_assembly_event_fire_when_async_action_and_passing_predicate_configured_with_sync_execute_while_selecting_entity()
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterInsertAssembly(async _ => { actionExecuted = true; await Task.CompletedTask; }, p => true));
        ...
        public Task Does_after_insert_assembly_event_not_fire_when_async_action_and_failing_predicate_configured_with_sync_execute_while_selecting_entity()
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterInsertAssembly(async _ => { actionExecuted = true; await Task.CompletedTask; }, p => false));
        ...
        public async Task Can_after_insert_assembly_event_fired_with_cancellation_of_token_source_with_async_execute_cancel_successfully()
        {
            ...
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterInsertAssembly(_ => { completion.SetResult(null); source.Cancel(); }));

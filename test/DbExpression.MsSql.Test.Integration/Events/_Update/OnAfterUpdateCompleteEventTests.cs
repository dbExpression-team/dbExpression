...
namespace DbExpression.MsSql.Test.Integration.Events
{
    public class OnAfterUpdateCompleteEventTests : ResetDatabaseNotRequired
    {
        ...
        public Task Does_after_update_complete_event_fire_when_async_action_configured_with_sync_execute_while_updateing_entity()
        {
            ...
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterUpdateComplete(async _ => { actionExecuted = true; await Task.CompletedTask; }));
        ...
        public Task Does_after_update_complete_event_fire_when_async_action_and_passing_predicate_configured_with_sync_execute_while_updateing_entity()
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterUpdateComplete(async _ => { actionExecuted = true; await Task.CompletedTask; }, p => true));
        ...
        public Task Does_after_update_complete_event_not_fire_when_async_action_and_failing_predicate_configured_with_sync_execute_while_updateing_entity()
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<v2019MsSqlDb>(configure => configure.Events.OnAfterUpdateComplete(async _ => { actionExecuted = true; await Task.CompletedTask; }, p => false));

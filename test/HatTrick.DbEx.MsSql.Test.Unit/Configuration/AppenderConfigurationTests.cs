...
namespace HatTrick.DbEx.MsSql.Test.Unit.Configuration
{
    public class AppenderConfigurationTests : TestBase
    {
        ...
        {
            //given & when & then
            var provider = ConfigureForMsSqlVersion(version, builder => builder.SqlStatements.Assembly.StatementAppender.Use(() => (IAppender)null!));
        ...
        public void Does_configuration_of_an_appender_factory_using_generic_use_method_succeed(int version)
        {
            ...
            var provider = ConfigureForMsSqlVersion(version, builder => builder.SqlStatements.Assembly.StatementAppender.Use<NoOpAppender>());

            //when
            var matchingType = provider.GetRequiredService<IAppender>();
        ...
        public void An_appender_factory_registered_via_delegate_should_be_correct_appender(int version)
        {
            ...
            var provider = ConfigureForMsSqlVersion(version, c => c.SqlStatements.Assembly.StatementAppender.Use(sp => appender));

            //when
            var resolved = provider.GetRequiredService<IAppender>();
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void An_appender_factory_registered_via_service_provider_should_be_correct_appender(int version)
        {
            //given
            var appender = Substitute.For<IAppender>();
            var provider = ConfigureForMsSqlVersion(version, c => c.SqlStatements.Assembly.StatementAppender.Use(sp => appender));
        ...
        public void An_appender_registered_via_generic_should_be_correct_appender_type(int version)
        {
            //given
            var appender = Substitute.For<IAppender>();
            var provider = ConfigureForMsSqlVersion(version, c => c.SqlStatements.Assembly.StatementAppender.Use<Appender>());
        ...
        public void An_appender_registered_via_generic_should_be_transient(int version)
        {
            ...
            //when
            var a1 = provider.GetRequiredService<IAppender>();
            var a2 = provider.GetRequiredService<IAppender>();
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void An_appender_registered_via_service_provider_and_instance_should_be_transient(int version)
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, c => c.SqlStatements.Assembly.StatementAppender.Use(sp => new NoOpAppender()));
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, c => c.SqlStatements.Assembly.StatementAppender.Use(() => new NoOpAppender()));

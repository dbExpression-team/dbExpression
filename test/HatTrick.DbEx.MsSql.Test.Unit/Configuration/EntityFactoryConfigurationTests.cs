...
namespace HatTrick.DbEx.MsSql.Test.Unit.Configuration
{
    public class EntityFactoryConfigurationTests : TestBase
    {
        ...
        {
            //given & when & then
            Assert.Throws<DbExpressionConfigurationException>(() => Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use((IEntityFactory)null!)));
        }
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use<NoOpEntityFactory>());

            //when
            var factory = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>() is NoOpEntityFactory;
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => new Person()));

            //when
            var entity = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>().CreateEntity<Person>();
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => sp.GetRequiredService<Person>(), c => c.ForEntityType<Person>().Use(() => new Person() { FirstName = "override" })));
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use(new NoOpEntityFactory()));
        ...
        public void Does_configuration_of_a_entity_creation_factory_using_default_use_method_succeed(int version)
        {
            ...
            //when
            var factory = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>() is not null;
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.ForEntityTypes(x => x.ForEntityType<Person>().Use(sp => new Person { FirstName = "UseThis" })));

            //when
            var person = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>().CreateEntity<Person>();
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Mapping.Use(new NoOpMapperFactory()));
        ...
        public void Does_configuration_of_a_entity_mapping_factory_using_default_use_method_succeed(int version)
        {
            ...
            //when
            var factory = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IMapperFactory>() is not null;
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.ForEntityTypes(x => x.ForEntityType<Person>().Use(sp => new Person { FirstName = "UseDefaultFactory" })));
        ...
        public void Does_configuration_of_a_entity_creation_factory_using_service_serviceProvider_and_new_instance_from_factory_resolve_as_transient(int version)
        {
            ...
            //when
            var a1 = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>();
        ...
        public void Does_configuration_of_a_entity_creation_factory_regsistered_with_factory_with_override_for_specific_entity_type_resolve_correctly_and_as_transient(int version)
        {
            ...
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use(sp => factory, c => c.ForEntityType<Person>().Use(() => new Person { FirstName = "x" })));
        ...
        public void An_entity_resolved_via_service_serviceProvider_and_type_should_be_transient(int version)
        {
            ...
            //when
            var a1 = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>().CreateEntity<Person>();
            var a2 = serviceProvider.GetServiceProviderFor<MsSqlDb>().GetRequiredService<IEntityFactory>().CreateEntity<Person>();
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => new Person() { FirstName = "base" }, c => c.ForEntityType<Person>().Use(() => new Person() { FirstName = "override" })));

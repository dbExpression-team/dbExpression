...
namespace HatTrick.DbEx.MsSql.Test.Unit.Configuration
{
    public class EntityFactoryConfigurationTests : TestBase
    {
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use<NoOpEntityFactory>());

            //when
            var factory = provider.GetRequiredService<IEntityFactory<MsSqlDb>>() is NoOpEntityFactory;
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void Does_configuration_of_a_entity_creation_factory_using_service_provider_and_new_instance_from_factory_succeed(int version)
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use(sp => new NoOpEntityFactory()));
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void Does_configuration_of_a_entity_creation_factory_using_service_provider_and_type_returning_new_instance_of_entity_should_succeed(int version)
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => new Person()));

            //when
            var entity = provider.GetRequiredService<IEntityFactory<MsSqlDb>>().CreateEntity<Person>();
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void Does_configuration_of_a_entity_creation_factory_using_service_provider_and_type_with_additional_override_resolve_correctly(int version)
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => sp.GetRequiredService<Person>(), c => c.ForEntityType<Person>().Use(() => new Person() { FirstName = "override" })));
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use(new NoOpEntityFactory()));
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void Does_configuration_of_a_entity_creation_factory_using_service_provider_and_instance_method_succeed(int version)
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use(sp => new NoOpEntityFactory()));
        ...
        public void Does_configuration_of_a_entity_creation_factory_using_default_use_method_succeed(int version)
        {
            ...
            //when
            var factory = provider.GetRequiredService<IEntityFactory<MsSqlDb>>() is not null;
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.ForEntityTypes(x => x.ForEntityType<Person>().Use(sp => new Person { FirstName = "UseThis" })));

            //when
            var person = provider.GetRequiredService<IEntityFactory<MsSqlDb>>().CreateEntity<Person>();
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Mapping.Use(new NoOpMapperFactory()));
        ...
        public void Does_configuration_of_a_entity_mapping_factory_using_default_use_method_succeed(int version)
        {
            ...
            //when
            var factory = provider.GetRequiredService<IMapperFactory<MsSqlDb>>() is not null;
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.ForEntityTypes(x => x.ForEntityType<Person>().Use(sp => new Person { FirstName = "UseDefaultFactory" })));
        ...
        public void Does_configuration_of_a_entity_creation_factory_using_service_provider_and_new_instance_from_factory_resolve_as_transient(int version)
        {
            ...
            //when
            var a1 = provider.GetRequiredService<IEntityFactory<MsSqlDb>>();
            var a2 = provider.GetRequiredService<IEntityFactory<MsSqlDb>>();
        ...
        public void Does_configuration_of_a_entity_creation_factory_regsistered_with_factory_with_override_for_specific_entity_type_resolve_correctly_and_as_transient(int version)
        {
            ...
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use(sp => factory, c => c.ForEntityType<Person>().Use(() => new Person { FirstName = "x" })));

            //when
            var a1 = provider.GetRequiredService<Person>();
            var a2 = provider.GetRequiredService<Person>();
        ...
        [Theory]
        [MsSqlVersions.AllVersions]
        public void An_entity_resolved_via_service_provider_and_type_should_be_transient(int version)
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => new Person()));
        ...
        {
            //given
            var provider = ConfigureForMsSqlVersion(version, builder => builder.Entities.Creation.Use((sp, t) => new Person() { FirstName = "base" }, c => c.ForEntityType<Person>().Use(() => new Person() { FirstName = "override" })));

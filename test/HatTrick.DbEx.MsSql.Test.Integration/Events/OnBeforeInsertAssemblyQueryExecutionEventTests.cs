...
{
    public class OnBeforeInsertAssemblyQueryExecutionEventTests : ResetDatabaseAfterEveryTest
    {
        [Theory]
        ...
        public void Does_before_update_assembly_query_execution_event_fire_when_sync_action_configured_with_sync_execute_while_selecting_entity(int version)
        {
            ...
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => actionExecuted = true));
        ...
        public void Does_before_update_assembly_query_execution_event_fire_expected_number_of_times_when_sync_action_configured_with_sync_execute_while_selecting_entity(int version, int expected = 5)
        {
            ...
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => actionExecutedCount++));
        ...
        public void Does_before_update_assembly_query_execution_event_fire_when_sync_action_and_passing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            var actionExecuted = false;
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => actionExecuted = true, p => true));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        public void Does_before_update_assembly_query_execution_event_not_fire_when_sync_action_and_failing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            var actionExecuted = false;
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => actionExecuted = true, p => false));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        public Task Does_before_update_assembly_query_execution_event_fire_when_async_action_configured_with_sync_execute_while_selecting_entity(int version)
        {
            ...
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(async _ => await Task.Run(() => actionExecuted = true)));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        public Task Does_before_update_assembly_query_execution_event_fire_when_async_action_and_passing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(async _ => await Task.Run(() => actionExecuted = true), p => true));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        public Task Does_before_update_assembly_query_execution_event_not_fire_when_async_action_and_failing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(async _ => await Task.Run(() => actionExecuted = true), p => false));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => throw new NotImplementedException()));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        public void Does_before_update_assembly_query_execution_event_throw_exception_when_fired_with_sync_action_and_predicate_throwing_exception_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => actionExecuted = true, _ => throw new NotImplementedException()));
            var person = new Person { FirstName = "xxx", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow };
        ...
        public async Task Can_before_update_assembly_query_execution_event_fired_with_cancellation_of_token_source_with_async_execute_cancel_successfully(int version)
        {
            ...
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertSqlStatementAssembly(_ => source.Cancel()));

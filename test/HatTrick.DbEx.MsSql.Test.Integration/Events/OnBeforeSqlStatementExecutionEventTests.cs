...
namespace HatTrick.DbEx.MsSql.Test.Integration.Events
{
    public class OnBeforeSqlStatementExecutionEventTests : ExecutorTestBase
    {
        ...
        public void Does_before_sql_statement_execution_event_fire_when_sync_action_configured_with_sync_execute_while_selecting_entity(int version)
        {
            ...
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => actionExecuted = true));
        ...
        public void Does_before_sql_statement_execution_event_fire_expected_number_of_times_when_sync_action_configured_with_sync_execute_while_selecting_entity(int version, int expected = 5)
        {
            ...
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => actionExecutedCount++));
        ...
        public void Does_before_sql_statement_execution_event_fire_when_sync_action_and_passing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            var actionExecuted = false;
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => actionExecuted = true, p => true));
        ...
        public void Does_before_sql_statement_execution_event_not_fire_when_sync_action_and_failing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            var actionExecuted = false;
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => actionExecuted = true, p => false));
        ...
        public Task Does_before_sql_statement_execution_event_fire_when_async_action_configured_with_sync_execute_while_selecting_entity(int version)
        {
            ...
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(async _ => await Task.Run(() => actionExecuted = true)));
        ...
        public Task Does_before_sql_statement_execution_event_fire_when_async_action_and_passing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(async _ => await Task.Run(() => actionExecuted = true), p => true));
        ...
        public Task Does_before_sql_statement_execution_event_not_fire_when_async_action_and_failing_predicate_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(async _ => await Task.Run(() => actionExecuted = true), p => false));
        ...
        {
            //given
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => throw new NotImplementedException()));
        ...
        public void Does_before_sql_statement_execution_event_throw_exception_when_fired_with_sync_action_and_predicate_throwing_exception_configured_with_sync_execute_while_selecting_entity(int version)
        {
            //given
            bool actionExecuted = false;
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => actionExecuted = true, _ => throw new NotImplementedException()));
        ...
        public async Task Can_before_sql_statement_execution_event_fired_with_cancellation_of_token_source_with_async_execute_cancel_successfully(int version)
        {
            ...
            var (db, serviceProvider) = ConfigureForMsSqlVersion(version, configure => configure.Events.OnBeforeSqlStatementExecution(_ => source.Cancel()));

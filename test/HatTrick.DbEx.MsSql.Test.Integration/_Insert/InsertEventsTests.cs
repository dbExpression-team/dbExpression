...
namespace HatTrick.DbEx.MsSql.Test.Integration
{
    ...
    public partial class InsertEventsTests : ResetDatabaseAfterEveryTest
    {
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(nameof(dbo.Person.FirstName), expected)));
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(dbo.Person.FirstName, expected)));
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(dbo.Product.Name, "ZZZ")));
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(nameof(dbo.Person.BirthDate), dbex.Null)));
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(dbo.Person.GenderType, expected)));
        ...
        public async Task Can_set_nullable_enum_field_value_to_an_enum_before_insert_assembly_event(int version) //this causes exception prior to test run: ", AddressType? expected = AddressType.Mailing)"
        {
            ...
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(dbo.Address.AddressType, expected)));
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(dbo.Address.AddressType, dbex.Null)));
            var address = new Address { AddressType = AddressType.Mailing, Line1 = "...", City = "YYY", State = "TX", Zip = "55555" };
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(nameof(dbo.Person.GenderType), expected)));
            var person = new Person { FirstName = "...", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow, BirthDate = DateTime.UtcNow };
        ...
        public async Task Can_set_field_value_by_name_for_a_field_that_is_design_time_configured_to_not_allow_insert(int version)
        {
            ...
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(nameof(dbo.Person.DateCreated), expected)));
            var person = new Person { FirstName = "...", LastName = "YYY", GenderType = GenderType.Female, RegistrationDate = DateTimeOffset.UtcNow, BirthDate = DateTime.UtcNow };
        ...
        {
            //given
            var (db, serviceProvider) = Configure<MsSqlDb>().ForMsSqlVersion(version, configure => configure.Events.OnBeforeInsertStart(context => context.SetFieldValue(dbo.Product.Name, expected)));
